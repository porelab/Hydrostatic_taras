...
public class NSelectproject1 implements Initializable {
	...
	void setAllThings() {
		...
		cmbselectind.setItems(FXCollections.observableArrayList("Protective Wear","Biotech",
		...
		cmbselectind.valueProperty().addListener(new ChangeListener<String>() {
			...
			public void changed(ObservableValue<? extends String> observable,
				...
				else if (newValue.equals("Protective Wear")) {
					cmbmateriala.setVisible(true);
					cmbmateriala.setItems(FXCollections
							.observableArrayList("PPE","Mask"));
				}
		...
		cmbselectind.getSelectionModel().select(0);
	}
	...
	void getFluiddata() {
		...
		try {
			...
			cmbselectflui.getSelectionModel().select(1);
		} catch (Exception e) {
			Exception ss;
		}
	...
	void nextClick() {
		...
		if (isvalid) {
			System.out.println("Old isze : "+(oldpos+1));
			if (oldpos + 1 > 0) {
	...
	void setTestConfiguration2() {
		...
		setToggleButtonProperty(tc_21, "Mode 1",
				"/userinput/firstbubble.png", t2group);
		setToggleButtonProperty(tc_22, "Mode 2",
				"/userinput/moderatebubble.png", t2group);
		setToggleButtonProperty(tc_23, "Mode 3",
	...
	void LoadProjectData(String projectname) {
		...
		System.out.println("Read mode------------>"+thresold);
		if (thresold.equals("Mode 1")) {
			t2group.selectToggle(tc_11);
			Myapp.thresold = "Mode 1";
			System.out.println("1----First Bu------------>"+thresold);
		} else if (thresold.equals("Mode 2")) {
			t2group.selectToggle(tc_12);
			Myapp.thresold = "Mode 1";
			System.out.println("2----First Bu------------>"+thresold);
		} else if (thresold.equals("Mode 3")) {
